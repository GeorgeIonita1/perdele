...
import { fallbackText, getSingleProduct } from "@/lib/utils";

async function ProductDetailPage({ params }: { params: { id: string } }) {
    const parameter = Number(params.id);
    const product = await getSingleProduct(parameter)

    return (
        <div className="container md:grid md:grid-cols-12 md:gap-16">
